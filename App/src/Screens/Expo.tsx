...
import { getExpo } from "../services/expo";
...
const Expo = ({title}:ExpoProps)=>{
    ...
    useEffect(()=>{
        ...
        async function fetchData(){
            if(params.exponame || title){
            const data = await getExpo({expo: title ?? params.exponame })
    ...
    const element = useMemo(()=>{
        if(params.exponame || title){
            return <PaintingGrid expo={title ?? params.exponame} />
        }else{
            return <></>
        }
    },[params,title])

    return <div className="relative flex flex-col overflow-auto z-[30] custom-scrollbar" id={"scrollbar"}>
        ...
        <div className="w-3/4 lg:w-2/3 h-[270px] lg:h-[520px] flex flex-col gap-2 center font-mt-bold text-justify text-[8px] sm:text-[16px]">
            ...
            <div className="flex flex-col w-[98%] sm:w-[90%] gap-4 text-3xs lg:text-[18px] text-textColor">
                {paragraphes && paragraphes.map((paragraphe:string)=><p>{paragraphe}</p>)}
            </div>

        
        </div>
        <div className="w-1/4 lg:w-1/3 h-[280px] sm:h-[520px] rounded-2xl flex center mt-8 relative flex center p-2 lg:p-0">
            <img id={"guillaume_picture"}src={"/images/guillaume.webp"} alt={"guillaume"} className="w-fit h-[80px] sm:h-[450px] rounded-lg sm:rounded-[4rem] "/>
        </div>
